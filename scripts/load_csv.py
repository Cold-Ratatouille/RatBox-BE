...
from app.ingestion.schemas import TABLE_INGREDIENTS_MASTER, TABLE_RECIPE_INGREDIENTS, TABLE_RECIPES
...
def main() -> None:
    ...
    # 마스터 데이터를 먼저 적재해야 recipe_ingredients의 참조가 유효함.
    # recipes/ingredients_master는 unique 컬럼 기준으로 upsert하여 재실행 시 중복 생성을 막음.
    ...
    print(
        f"레시피 {len(result.recipes)}개, 표준 재료 {len(result.ingredients_master)}개, "
